...
{
void abortInit(std::string const& errorMsg, bool finalize = true)
{
...
std::string getSerialOfFirstCamera(ros::NodeHandle& nh, std::string const& cameraNodeType)
{
  ...
  {
    if (cameras[i][itmType].exists() && cameras[i][itmType].asString() == cameraNodeType)
    {
...
std::unique_ptr<CameraType> initCamera(ros::NodeHandle& nh, ros::NodeHandle& nhPrivate, std::string const& nodeType)
{
  ...
  std::unique_ptr<CameraType> camera = ::make_unique<CameraType>(nh, std::move(params));
